...
void SlamVisualizer::pushViewMatrix(std::vector<FrameGL>& v_Frames)
{
	...
	std::for_each(m_vFrames.begin(), m_vFrames.end(), []( FrameGL& frame) {frame.m_type = 0; });
...
void SlamVisualizer::pushImageTexture(unsigned char* imageTexture, const int width, const int height,
	const int channels)
{
...
void SlamVisualizer::pushWorldPoint(const std::vector<WorldPointGL>& v_Point3f)
{
...
SlamVisualizer::SlamVisualizer(int width, int height) :m_width(width), m_height(height)
{
	m_channels = 3;
...
void SlamVisualizer::render()
{
	...
	pangolin::CreateWindowAndBind("SlamVisualizer", m_width * 2 + UI_WIDTH, m_height);
	...
	m_viewMatrix.SetProjectionMatrix(pangolin::ProjectionMatrix(640, 480, 420, 420, 320, 240, 0.1, 100000));
	//m_viewMatrix.SetModelViewMatrix(pangolin::ModelViewLookAt(-0, 0, -100, 0, 0, 0, pangolin::AxisY));
	...
	view3D.SetBounds(0., 1., pangolin::Attach::Pix(UI_WIDTH), pangolin::Attach::Pix(UI_WIDTH + m_width), 640. / 480.).SetHandler(&handler3D);

	View& viewImageTexture = pangolin::Display("ImageTexture");

	viewImageTexture.SetBounds(0., 1., pangolin::Attach::Pix(UI_WIDTH + m_width), 1., -640. / 480.).SetLock(pangolin::LockRight, pangolin::LockBottom);
	...
	pangolin::Var<std::function<void(void)> > Reset("ui.RecordVideo", RecordVideo);

	// Demonstration of how we can register a keyboard hook to alter a Var
	pangolin::RegisterKeyPressCallback('f', pangolin::ToggleVarFunctor("ui.A_Button"));
...
void SlamVisualizer::_drawImageTexture()
{
	...
	m_GLTexture.Reinitialise(m_width, m_height, GL_RGB, 0, 0, GL_BGR, GL_UNSIGNED_BYTE, m_pImageTexture);
...
void SlamVisualizer::_drawGeometry()
{
	...
	if (FollowCamera.Get()) {
		m_FollowMatrix.SetModelViewMatrix(m_viewMatrix.GetModelViewMatrix() * T_wc.Inverse());
...
void SlamVisualizer::_drawCameraFrustum()
{
	...
	for (int i = 1, n = m_vFrames.size(); i < n; ++i) {
		glLoadIdentity();
		glMultMatrixd((m_FollowMatrix.GetModelViewMatrix() * OpenGlMatrix::ColMajor4x4(m_vFrames[i].m_Twc.data())).m);
...
void SlamVisualizer::detach()
{
	m_thread->detach();
}
const float world_axis[] = { 0, 0, 0, 100, 0, 0, 0, 0, 0, 0, 100, 0, 0, 0, 0, 0, 0, 100 };
...
void SlamVisualizer::_saveFrames()
{
	...
	static cv::VideoWriter video1("VideoTest.avi", CV_FOURCC('M', 'J', 'P', 'G'), 25.0, cv::Size(v1.w, v1.h));
	static cv::VideoWriter video2("VideoTest2.avi", CV_FOURCC('M', 'J', 'P', 'G'), 25.0, cv::Size(v2.w, v2.h));
