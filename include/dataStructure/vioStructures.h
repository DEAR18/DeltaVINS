...
struct VisualObservation {
    ...
    VisualObservation(const Vector2f& px, const Vector3f& ray, Frame* frame)
        : m_px(px), m_Ray(ray), m_linkFrame(frame) {}
...
struct Frame {
    ...
    std::unordered_set<TrackedFeature*>
        m_vTrackedFeatures;  // All tracked feature in this frame.
    long long timestamp;
...
struct TrackedFeature : public NonLinear_LM<3, float> {
    ...
    std::vector<VisualObservation>
        m_vVisualObs;  // vector of all visual observations
    Vector2f
        m_PredictedPx;  // predicted pixel position using propagated camera pose
