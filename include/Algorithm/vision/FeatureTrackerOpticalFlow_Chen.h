...
namespace DeltaVins
{
    ...
    public:
        ...
        void matchNewFrame(std::list<TrackedFeaturePtr>& vTrackedFeatures, cv::Mat& image, Frame* camState);

        ~FeatureTrackerOpticalFlow_Chen();
    private:

        void _extractFast(const int imgStride, const int halfMaskSize, std::vector<cv::Point2f>& vTemp);
