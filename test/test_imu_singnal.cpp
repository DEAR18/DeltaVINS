...
std::vector<uint8_t> start_command{0x59, 0xe1, 0x22, 0x01, 0x00,
                                   0x00, 0x11, 0x12, 0x47};
...
void convert(uint8_t *data) {
    ...
    {
        if (data[T] == 0x41) {
            int dt = (data[T + 2]) | (data[T + 3] << 8) | (data[T + 4] << 16) |
                     (data[T + 5] << 24);
            int imu_nums = (data[T + 6]) | (data[T + 7] << 8) |
                           (data[T + 8] << 16) | (data[T + 9] << 24);
            int camera_plus_flag = (data[T + 10]) | (data[T + 11] << 8) |
                                   (data[T + 12] << 16) | (data[T + 13] << 24);
            ...
            printf("timestamp : %llu dt:%d ,idx:%d cam_flag:%d,camFlagCnt%d ",
                   sum_t, dt, imu_nums, camera_plus_flag, camFlagCnt);
        ...
        if (data[W] == 0x20) {
            int wx = (data[W + 2]) | (data[W + 3] << 8) | (data[W + 4] << 16) |
                     (data[W + 5] << 24);
            int wy = (data[W + 6]) | (data[W + 7] << 8) | (data[W + 8] << 16) |
                     (data[W + 9] << 24);
            int wz = (data[W + 10]) | (data[W + 11] << 8) |
                     (data[W + 12] << 16) | (data[W + 13] << 24);
        ...
        if (data[A] == 0x10) {
            int ax = (data[A + 2]) | (data[A + 3] << 8) | (data[A + 4] << 16) |
                     (data[A + 5] << 24);
            int ay = (data[A + 6]) | (data[A + 7] << 8) | (data[A + 8] << 16) |
                     (data[A + 9] << 24);
            int az = (data[A + 10]) | (data[A + 11] << 8) |
                     (data[A + 12] << 16) | (data[A + 13] << 24);
            float axf = ax * s;
            float ayf = ay * s;
            float azf = az * s;
            printf("\nacc : %f , %f , %f \n", axf, ayf, azf);
...
int main(int argc, char **argv) {
    ...
    if (argc > 1) timeAll = strtoll(argv[1], nullptr, 10);
    ...
    auto m_serial =
        new serial::Serial(port, baud, serial::Timeout::simpleTimeout(10));
    ...
    int count = 0;
    // Todo: To make it more faster, erase and insert function of vector is too
    // slow.
    while (true) {
        ...
        while (lastHeader + 68 < data.size()) {
            if (data[67 + lastHeader] == 0x59 &&
                data[68 + lastHeader] == 0x53) {
