...
inline void FeatureTrackerOpticalFlow_Chen::_setMask(int x, int y) {
    ...
    unsigned char* pMask0 =
        m_pMask + x + (y - halfMaskSize) * imgStride - halfMaskSize;
...
void FeatureTrackerOpticalFlow_Chen::_extractMorePoints(
    ...
    for (int i = 0, j = m_nMax2Track - m_nTracked; i < corners.size() && j > 0;
         ++i) {
        int x = corners[i].x;
        int y = corners[i].y;
...
void FeatureTrackerOpticalFlow_Chen::_trackPoints(
    ...
    Matrix3f dR =
        m_pCamState->state->m_Rwi.transpose() * m_pCamState0->state->m_Rwi;
    ...
    cv::calcOpticalFlowPyrLK(
        lastImage, m_image, pre, now, status, err, cv::Size(21, 21), 3,
...
void FeatureTrackerOpticalFlow_Chen::_extractFast(
    ...
    fast::fast_corner_detect_10_mask(
        m_image.data + halfMaskSize + halfMaskSize * imgStride,
        m_pMask + halfMaskSize + halfMaskSize * imgStride,
        m_image.cols - m_nMaskSize + 1, m_image.rows - m_nMaskSize + 1,
        m_image.step1(), 15, vXys);
    fast::fast_corner_score_10(
        m_image.data + halfMaskSize + halfMaskSize * imgStride, m_image.step1(),
        vXys, 15, vScores);
