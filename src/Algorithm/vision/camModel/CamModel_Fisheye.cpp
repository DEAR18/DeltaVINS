...
namespace DeltaVins
{
	OcamModel::OcamModel(float c_, float d_, float e_, float cx_, float cy_, float a0_, float a2_, float a3_,
	                     float a4_): cx(cx_), cy(cy_), c(c_), d(d_), e(e_)
	{
		poly[0] = a0_;
		poly[1] = 0;
		poly[2] = a2_;
	...
	OcamModel::OcamModel(float c_, float d_, float e_, float cx_, float cy_, float a0_, float a2_, float a3_, float a4_,
	                     float ia0_, float ia1_, float ia2_, float ia3_, float ia4_): cx(cx_), cy(cy_), c(c_), d(d_),
	                                                                                  e(e_)
	{
		poly[0] = a0_;
		poly[1] = 0;
	...
	FisheyeModel::FisheyeModel(int width, int height, float cx, float cy, float c, float d, float e, float a0, float a2,
	                           float a3, float a4, bool aligment): CamModel(width, height),
	...
	FisheyeModel::FisheyeModel(int width, int height, float cx, float cy, float c, float d, float e, float a0, float a2,
	                           float a3, float a4, float ia0, float ia1, float ia2, float ia3, float ia4,
	                           bool aligment): CamModel(width, height),
	...
	FisheyeModel* FisheyeModel::createFromConfig(cv::FileStorage& config)
	{
		...
		if (D.cols == 7)
			p = new FisheyeModel(pK[0], pK[1], pK[2], pK[3], pD[0], pD[1], pD[2], pD[3], pD[4], pD[5], pD[6],
			                     alignment);
		else if (D.cols == 12)
			p = new FisheyeModel(pK[0], pK[1], pK[2], pK[3], pD[0], pD[1], pD[2], pD[3], pD[4], pD[5], pD[6], pD[7],
			                     pD[8], pD[9], pD[10], pD[11], alignment);
	...
	void FisheyeModel::computeInvPoly()
	{
		...
		printf("Inv Poly:%f %f %f %f %f\n", ocamModel.inv_poly[0], ocamModel.inv_poly[1], ocamModel.inv_poly[2],
		       ocamModel.inv_poly[3], ocamModel.inv_poly[4]);
	...
	Vector3d FisheyeModel::imageToCam(const Vector2d& px)
	{
		...
		if (alignment)
		{
			...
			xyz[0] = invdet * ((px.x() - ocamModel.cx) - ocamModel.d * (px.y() - ocamModel.cy));
			xyz[1] = invdet * (-ocamModel.e * (px.x() - ocamModel.cx) + ocamModel.c * (px.y() - ocamModel.cy));
		}
		else
		{
		...
		double rho = xyz.head<2>().norm(); //distance [pixels] of  the point from the image center
	...
	Vector3f FisheyeModel::imageToCam(const Vector2f& px)
	{
		...
		float rho = xyz.head<2>().norm(); //distance [pixels] of  the point from the image center
		xyz[2] = ocamModel.poly[0];
		float r_i = 1;
