...
namespace DeltaVins
{
	...
	void EquiDistantModel::calibrate(cv::FileStorage& config)
	{
		...
		for (int i=0;i< m_imagePoints.size();++i)
		{
			...
			for (int j = 0; j < m_objectPoints.size(); ++j) {
				...
				opv.emplace_back(m_objectPoints[j].x(), m_objectPoints[j].y(), m_objectPoints[j].z());
			}
		}
		for (int i=0;i<m_imagePoints.size();++i)
		{
		...
		double rms = cv::fisheye::calibrate(objPoints, imagePoints, cv::Size(640, 480), K, D, rvecs, tvecs, cv::fisheye::CALIB_CHECK_COND|cv::fisheye::CALIB_RECOMPUTE_EXTRINSIC, cv::TermCriteria(cv::TermCriteria::COUNT + cv::TermCriteria::EPS, 100, DBL_EPSILON));

		printf("k1:%lf,k2:%lf,k3:%lf,k4:%lf\n",D.at<double>(0),D.at<double>(1),D.at<double>(2),D.at<double>(3));
	...
	void EquiDistantModel::computeInvPoly(){
	    ...
	    for(float rad = 0.01;rad<M_PI_2;rad+=0.01){
	        float theta_d = rad*(1+k[0]*powf(rad,2)+k[1]*powf(rad,4)+k[2]*powf(rad,6)+k[3]*powf(rad,8));
	...
	Vector2f EquiDistantModel::camToImage(const Vector3f& pCam, Matrix23f& J23)
	{
		...
		theta_exp[0] = theta;
		for (int i = 0; i < 9; ++i)
			theta_exp[i + 1] = theta * theta_exp[i];
		float dtd = 1 + 3 * k[0] * theta_exp[2] + 5 * k[1] * theta_exp[4] + 7 * k[2] * theta_exp[6] + 9 * k[3] * theta_exp[8];
		float td = theta + k[0] * theta_exp[3] + k[1] * theta_exp[5] + k[2] * theta_exp[7] + k[3] * theta_exp[9];
