                           ...
                           short barrier, std::vector<fast_xy>& corners)
{
  ...
  for(y = 3 ; y < img_height - 3; y++)
  {
    ...
    for(; cache_0 < line_max;cache_0++)
    {
            ...
            success:
                    corners.emplace_back(static_cast<short>(cache_0-line_min),
                                              static_cast<short>(y));
    }
  }
}
void fast_corner_detect_10_mask(const fast_byte* img,const fast_byte* mask, int img_width, int img_height, int img_stride,
	short barrier, std::vector<fast_xy>& corners)
{
	int y, cb, c_b;
	const fast_byte* line_max, * line_min;
	const fast_byte* cache_0;
	const fast_byte* mask_0;
	...
	for (y = 3; y < img_height - 3; y++)
	{
		...
		for (; cache_0 < line_max; cache_0++,mask_0++)
		{
			if (!*mask_0) continue;
			cb = *cache_0 + barrier;
			c_b = *cache_0 - barrier;
