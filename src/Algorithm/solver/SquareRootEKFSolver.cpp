...
namespace DeltaVins
{
    ...
    )
    {
        ...
        m_n = n;
#endif
    	m_infoFactorMatrix.topLeftCorner(CURRENT_DIM, CURRENT_DIM) = p.cwiseSqrt().cwiseInverse().asDiagonal();
    ...
    void SquareRootEKFSolver::propagateStatic(const ImuPreintergration *pImuTerm){
        ...
        *m_pVel += R0 * pImuTerm->dV + gravity * dt;
        ...
        static Eigen::Matrix<float,NEW_STATE_DIM,NEW_STATE_DIM> StateTransitionMatrix;
        ...
        NoiseFactor.diagonal().segment<3>(6) = Vector3f::Ones()* (1.f / (sqrt(Config::GyroBiasNoise2) * dt));
        NoiseFactor.diagonal().segment<3>(12) = Vector3f::Ones()*(1.f / (sqrt(Config::AccBiasNoise2) * dt));
        ...
        NoiseCov = NoiseTransitionMatrix * pImuTerm->Cov * NoiseTransitionMatrix.transpose();
        ...
        m_infoFactorMatrix.block<NEW_STATE_DIM, IMU_STATE_DIM>(OLD_DIM, IMUIdx) = R.rightCols<IMU_STATE_DIM>();
        m_infoFactorMatrix.block<NEW_STATE_DIM, CAM_STATE_DIM>(OLD_DIM, OLD_DIM - CAM_STATE_DIM) = R.leftCols<CAM_STATE_DIM>();
        //m_infoFactorMatrix.block<NEW_STATE_DIM, NEW_STATE_DIM>(N, N - NEW_STATE_DIM) = NoiseFactor * StateTransitionMatrix;
        m_infoFactorMatrix.block<NEW_STATE_DIM, NEW_STATE_DIM>(OLD_DIM,OLD_DIM) = -NoiseFactor;
    ...
    bool SquareRootEKFSolver::MahalanobisTest(PointState* state)
    {
...
#endif
        	E.leftCols<3>() = m_infoFactorMatrixAfterMarigin.block(0,state->m_idx * 3 + iLeft,CURRENT_DIM,3);
            E.rightCols<6>() = m_infoFactorMatrixAfterMarigin.block(0,state->host->m_vVisualObs.back().m_linkFrame->state->m_idx * CAM_STATE_DIM + iLeft + 3 * m_vSlamPoint.size(),CURRENT_DIM,6);
            Matrix9f F = E.transpose() * E;
            S = state->H.leftCols(9)* F.inverse()* state->H.leftCols(9).transpose();
            S.noalias() += R;
        ...
        MatrixXf B = state->H.leftCols(nExceptPoint) * m_infoFactorInverseMatrix.topLeftCorner(
                nExceptPoint,
    ...
    int SquareRootEKFSolver::computeJacobians(TrackedFeature* track)
    {
        ...
        auto calcObsJac = [&](VisualObservation* ob)
        {
            ...
            Vector3f Pi = Riw * (track->m_pState->m_Pw - ob->m_linkFrame->state->m_Pwi);
            Vector3f Pi_FEJ = Riw * (track->m_pState->m_Pw_FEJ - ob->m_linkFrame->state->m_Pw_FEJ);
            ...
            H.block<2, 3>(2 * index, camStateIdx + CAM_STATE_DIM * cam_id) = J23 *  crossMat(Pi_FEJ);
#if USE_POSITION_DETECT_ROTATION
        	if(!track->m_bInaccurateDepth)
#endif
            H.block<2, 3>(2 * index, camStateIdx + CAM_STATE_DIM * cam_id + 3) = -J23 *  Riw;
    ...
    int SquareRootEKFSolver::_addNewSlamPointConstraint()
    {
        ...
        {
            m_infoFactorMatrix.topLeftCorner(CURRENT_DIM, CURRENT_DIM) = m_infoFactorMatrixAfterMarigin.topLeftCorner(CURRENT_DIM, CURRENT_DIM);
            return nObs;
        }
        ...
        m_infoFactorMatrix.topLeftCorner(nLeft, nLeft) = m_infoFactorMatrixAfterMarigin.topLeftCorner(nLeft, nLeft);
        m_infoFactorMatrix.block(0, nNewCam, nLeft, nRight) = m_infoFactorMatrixAfterMarigin.block(0, nLeft, nLeft, nRight);
        m_infoFactorMatrix.block(nNewCam, 0, nRight, nLeft) = m_infoFactorMatrixAfterMarigin.block(nLeft, 0, nRight, nLeft);
        m_infoFactorMatrix.block(nNewCam, nNewCam, nRight, nRight) = m_infoFactorMatrixAfterMarigin.block(nLeft, nLeft, nRight, nRight);

        m_infoFactorMatrix.block(0, nLeft, CURRENT_DIM + nNewSlamPointStates, nNewSlamPointStates).setZero();
        m_infoFactorMatrix.block(nLeft, 0, nNewSlamPointStates,CURRENT_DIM + nNewSlamPointStates).setZero();
        ...
        m_vSlamPoint.insert(m_vSlamPoint.end(), m_vNewSlamPoint.begin(), m_vNewSlamPoint.end());
    ...
    {
        constexpr int MAX_DIM = 3 + (CAM_STATE_DIM * MAX_WINDOW_SIZE + IMU_STATE_DIM + 1);
        static MatrixHfR H;
    ...
    void SquareRootEKFSolver::addVelocityConstraint(int nRows)
    {
        ...
        if (absAngle < FLT_EPSILON){
            ...
            Jr = Eigen::Matrix3f::Identity() + 0.5 * crossSo3 + (pow(1.f / absAngle, 2) - (1 + cos(absAngle) / (2 * absAngle * sin(absAngle)))) * crossSo3 * crossSo3;
            Jl = Jr.transpose();
        }
        m_StackedMatrix.block<3, 3>(nRows, CURRENT_DIM - 2 * CAM_STATE_DIM) = Jr * invRotSigma;
        m_StackedMatrix.block<3, 3>(nRows, CURRENT_DIM - CAM_STATE_DIM) = -Jl * invRotSigma;
...
#endif    	
        m_StackedMatrix.block<3, 3>(nRows + 3, CURRENT_DIM - 2 * CAM_STATE_DIM + 3) = -Matrix3f::Identity() * invPosSigma;
        m_StackedMatrix.block<3, 3>(nRows + 3, CURRENT_DIM - CAM_STATE_DIM + 3) = Matrix3f::Identity() * invPosSigma;
#if REMOVE_RESIDUAL_STACK
        m_obsResidual.segment(nRows + 3, 3) = -dP * invPosSigma;
#else
        m_StackedMatrix.block<3, 1>(nRows + 3, CURRENT_DIM) = -dP * invPosSigma;
#endif
        m_StackedMatrix.block<3, 3>(nRows+6, velIdx) = Matrix3f::Identity() * invSigma;
#if REMOVE_RESIDUAL_STACK
    ...
    int SquareRootEKFSolver::stackInformationFactorMatrix()
    {
        ...
        m_StackedMatrix.block(0, nCamStartIdx, nCams, CAM_STATE_DIM * nCams) = m_PlaneH.middleCols(3, CAM_STATE_DIM * nCams);
#if REMOVE_RESIDUAL_STACK
    	...
    	for (auto&track:m_vSlamPoint)
    	{
            ...
            m_StackedMatrix.block(rffIdx, nCamStartIdx, nObs, nCamStates) = track->H.middleCols(3, nCamStates);
            m_StackedMatrix.block(rffIdx, slamPointIdx + slamId * 3, nObs, 3) = track->H.leftCols<3>();
#if REMOVE_RESIDUAL_STACK
            m_obsResidual.segment(rffIdx,nObs) = track->H.rightCols<1>();
#else
            m_StackedMatrix.block(rffIdx, nOldStates, nObs, 1) = track->H.rightCols<1>();
    ...
    void SquareRootEKFSolver::solveAndUpdateStates()
    {
        ...
        if (haveNewInformation) {
            ...
            stackedMatrix.topLeftCorner(CURRENT_DIM, CURRENT_DIM) = m_infoFactorMatrix.topLeftCorner(CURRENT_DIM, CURRENT_DIM);
            ...
            m_infoFactorMatrix.topLeftCorner(CURRENT_DIM, CURRENT_DIM) =
                qr.matrixQR().topLeftCorner(CURRENT_DIM, CURRENT_DIM).triangularView<Eigen::Upper>();
            ...
            VectorXf dx = m_infoFactorMatrix.topLeftCorner(CURRENT_DIM,
                CURRENT_DIM).triangularView<Eigen::Upper>().solve(m_residual.segment(0, CURRENT_DIM));
            TickTock::stop("Inverse");

            int iDim = 0;

#if USE_PLANE_PRIOR
            printf("Plane coeff:%f %f %f -> ", m_planeCoeff->x(), m_planeCoeff->y(), m_planeCoeff->z());
            *m_planeCoeff += dx.segment<3>(0);
            printf("%f %f %f\n", m_planeCoeff->x(), m_planeCoeff->y(), m_planeCoeff->z());
            iDim += PLANE_DIM;
#endif
            ...
            *m_pVel += dx.segment<3>(iDim + 3);
            ...
            for (auto &camState : m_vCamStates) {
                camState->m_Rwi = camState->m_Rwi * Sophus::SO3Group<float>::exp(dx.segment<3>(iDim)).matrix();
                iDim += 3;
    ...
    void SquareRootEKFSolver::marginalizeGivens() {
        ...
        for (int i=0,n=m_vSlamPoint.size();i<n;++i)
        {
            if (m_vSlamPoint[i]->m_bToMargin||m_vSlamPoint[i]->m_bNextMargin) m_vSlamPoint[i]->host->m_bDead = true;
        ...
        for (auto idx : v_MarginDIM)
        {
            m_infoFactorMatrixToMarginal.col(index++).segment(0, CURRENT_DIM) = m_infoFactorMatrix.col(idx).segment(0,CURRENT_DIM);
        }
        for (auto idx : v_RemainDIM)
        {
            m_infoFactorMatrixToMarginal.col(index++).segment(0, CURRENT_DIM) = m_infoFactorMatrix.col(idx).segment(0, CURRENT_DIM);
        ...
        m_infoFactorMatrixAfterMarigin.topLeftCorner(CURRENT_DIM, CURRENT_DIM) = m_infoFactorMatrixToMarginal.block(OLD_DIM - CURRENT_DIM, OLD_DIM - CURRENT_DIM, CURRENT_DIM, CURRENT_DIM);

#else


        m_infoFactorMatrix.topLeftCorner(CURRENT_DIM, CURRENT_DIM) = m_infoFactorMatrixToMarginal.block(OLD_DIM - CURRENT_DIM, OLD_DIM - CURRENT_DIM, CURRENT_DIM, CURRENT_DIM);
    ...
    void SquareRootEKFSolver::marginalizeStatic()
    {
        ...
        Eigen::HouseholderQR<MatrixXf> qr(m_infoFactorMatrixToMarginal.topLeftCorner(CURRENT_DIM, CURRENT_DIM));
        CURRENT_DIM = v_RemainDIM.size();


        m_infoFactorMatrix.topLeftCorner(CURRENT_DIM, CURRENT_DIM) = qr.matrixQR().bottomRightCorner(CURRENT_DIM, CURRENT_DIM).triangularView<Eigen::Upper>();



        //m_infoFactorInverseMatrix = MatrixXf::Identity(CURRENT_DIM, CURRENT_DIM);
        //m_infoFactorMatrix.topLeftCorner(CURRENT_DIM, CURRENT_DIM).triangularView<Eigen::Upper>().solveInPlace(m_infoFactorInverseMatrix);
