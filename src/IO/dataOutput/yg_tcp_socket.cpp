                                  ...
                                  unsigned int recvTime)
{
    ...
    {
        LOGE("start tcp client error:%s.ip: %s port: %d",strerror(errno),ip,port);
        return false;
    }
...
int YG_TCP_Client::ConnectToServer()
{
    ...
    int resC = ::connect(socket_fd, (struct sockaddr*)&host_addr, sizeof(host_addr));
...
bool YG_TCP_Server::InitTcpServer(unsigned short port,unsigned int size)
{
    ...
    int res = bind(socket_server,(struct sockaddr*)&host_addr,sizeof(host_addr));
    if(res < 0)
    {
