...
void PoseOutputTcp::pushViewMatrix(std::vector<FrameGL> &v_Rcw) {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        memcpy(p + sizeof(uint32_t) * 2, v_Rcw[i].m_Twc.data(),
               sizeof(float) * 16);
        p += step;
    }
...
void PoseOutputTcp::pushImageTexture(unsigned char *imageTexture,
    ...
    counter++;
    {
        ...
        if (channels != this->channels || width != this->width ||
            height != this->height) {
...
void PoseOutputTcp::sendImage() {
    ...
    while (true) {
        ...
        tcpClient_imageTexture.SendData(reinterpret_cast<char *>(&header[0]),
                                        header.size());
        tcpClient_imageTexture.SendData(
            reinterpret_cast<char *>(&encoderImage[0]), encoderImage.size());
        tcpClient_imageTexture.SendData(reinterpret_cast<char *>(&end[0]),
                                        end.size());
    }
}
