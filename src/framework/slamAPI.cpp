...
void initSlamSystem(const char* datasetDir, const char* testName)
{
	...
	if(Config::RunVIO)
	    vioModulePtr = std::make_shared<VIOModule>();
    ...
    if(Config::RunVIO)
	    dataSourcePtr->addImageObserver(vioModulePtr.get());
...
#if ENABLE_VISUALIZER
	if (!Config::NoGUI && slamVisualizerPtr)
		slamVisualizerPtr->start();
#endif
	//join
	dataSourcePtr->start();
...
#if ENABLE_VISUALIZER
	if (slamVisualizerPtr)
		slamVisualizerPtr->join();
#endif
}
